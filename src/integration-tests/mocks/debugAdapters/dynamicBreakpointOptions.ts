...
class DynamicBreakpointOptionsGDBBackend extends GDBBackend {
    public async getBreakpointOptions(_: MIBreakpointLocation, initialOptions: MIBreakpointInsertOptions): Promise<MIBreakpointInsertOptions> {
        if (throwError) {
            throw new Error("Some error message providing information that the breakpoint is not valid!");
        }
        const hardware = hardwareBreakpointTrue ? true : hardwareBreakpointFalse ? false : initialOptions.hardware;
